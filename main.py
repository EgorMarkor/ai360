...
def tariff_text_intro() -> str:
    return "Выбери тариф под свой формат работы 👇"
...
def tariff_description(code: str) -> str:
    ...
    if code == "start":
        bullets = [
            "Маркетолог 24/7 (только текст)",
            "Стратегии, планы, офферы, воронки",
            "Контент-планы и тексты постов и объявлений",
            "Базовые сценарии",
            "Стратегия на 7/30 дней",
            "Доступ к базовым обучающим материалам",
        ]
    elif code == "marketing_pro":
        bullets = [
            "Всё из тарифа «Старт»",
            "Расширенные сценарии и детализированные стратегии",
            "Генерация изображений (креативы, обложки, баннеры)",
            "Лимиты: до 300 текстовых запросов и 50 изображений",
        ]
    elif code == "content_studio":
        bullets = [
            "Всё из «Маркетинг-про»",
            "Генерация видео: Reels/Shorts/VK-клипы и ролики до 3 минут",
            "Презентации: питчи, КП, вебинары (тексты слайдов + рекомендации)",
            "Лимиты: 600 текстов, 80 изображений, 15 видео-сценариев, 3 презентации",
        ]
    else:
        bullets = [
            "Всё из «Контент-студия»",
            "Массовая генерация под несколько проектов",
...
# ------------------------------
DIAG_QUESTIONS = [
    ("business_and_offer", "1️⃣ Что за бизнес и что ты продаёшь?\n(Кратко: ниша, продукт/услуга, для кого, средний чек)"),
    ("current_results", "2️⃣ Какие сейчас результаты?\n(Примерно: клиенты в месяц, выручка, стабильность)"),
    ("client_sources", "3️⃣ Откуда приходят клиенты?\n(Сарафан, соцсети, реклама, сайт, маркетплейсы и т.д.)"),
    ("customer_journey", "4️⃣ Как выглядит путь клиента?\n(Где видят → как связываются → как платят)"),
    ("marketing_goals", "5️⃣ Чего ты хочешь от маркетинга в ближайшие 3 месяца?\n(Больше клиентов, денег, стабильности, системы и т.п.)"),
]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "— выстраивать системный маркетинг без хаоса.\n\n"
        "360° — это архитектура роста твоего бизнеса."
    )
    await update.message.reply_text(text, reply_markup=MAIN_MENU)

    await update.message.reply_text(
        "🎥 Посмотри короткое видео (2–3 минуты),\nгде я показываю, как могу быть полезен именно тебе."
    )

    await update.message.reply_text(
        "Предлагаю начать с экспресс-диагностики маркетинга твоего бизнеса.\n\n"
        "Формат:\n— 5 ключевых вопросов\n— 10–15 минут\n— без воды\n\n"
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Диагностика бизнеса" in txt or txt == "Пройти диагностику 🚀" or txt == "Начать диагностику 🚀" or txt == "🚀 НАЧАТЬ ДИАГНОСТИКУ":
    ...
    if st.stage == "diag_choice":
        await handle_diag_choice_input(update, context, txt)
        return
...
async def handle_diagnostic_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, txt: str):
    ...
    # Последний ответ получен — предлагаем варианты продолжения
    st.diagnostic_step = 0
    await present_post_diag_choices(update.message, st)
    return


async def present_post_diag_choices(message_obj, st: UserState):
    st.stage = "diag_choice"
    st.chat_mode = False
    await safe_reply_text(
        message_obj,
        "Спасибо, я разобрал твою ситуацию 👌\n\nКак идём дальше?",
        reply_markup=INLINE_POST_DIAG,
    )


async def run_demo_after_diagnostic(message_obj, st: UserState, *, bot=None, chat_id: Optional[int] = None):
    st.stage = "idle"
    prompt = (
        "Сделай экспресс-разбор маркетинга по ответам пользователя.\n"
        "Дай по делу: 1) Кратко о нише и модели 2) Сильные стороны 3) Слабые места/риски 4) Первые шаги на 7–14 дней "
        "5) Приоритеты на 30 дней (3 пункта). Стиль: экспертно, дружелюбно, без воды, без символов * или #.\n\n"
        f"Ответы пользователя (JSON): {json.dumps(st.answers, ensure_ascii=False)}"
    )
    analysis = await ask_gpt_with_typing(bot, chat_id, prompt)
    await send_gpt_reply(
        message_obj,
        st,
        "Экспресс-разбор готов 👇\n\n" + analysis,
    )
    await send_demo_value_message(message_obj)


async def send_demo_value_message(message_obj):
    value_text = (
        "Смотри, что сейчас произошло 👇\n\n"
        "За пару минут ты получил(а) то,\n"
        "за что обычный маркетолог берёт деньги:\n"
        "• разбор ниши,\n"
        "• сильные и слабые стороны,\n"
        "• первые шаги.\n\n"
        "И это была только облегчённая демо-версия.\n\n"
        "В платной версии ИИ-маркетолога 360 ты сможешь:\n"
        "• работать с маркетологом 24/7, без созвонов и ожиданий;\n"
        "• за минуты собирать стратегии на 7/30 дней, воронки и планы запусков;\n"
        "• получать готовые контент-планы и тексты: посты, сторис, письма, рекламные объявления;\n"
        "• на продвинутых тарифах — креативы, сценарии видео и презентации.\n\n"
        "По сути, ты получаешь:\n"
        "• маркетолога,\n"
        "• копирайтера,\n"
        "• контент-отдел\n\n"
        "В одном сервисе — по цене ниже одного дня работы хорошего специалиста.\n\n"
        "Выбери тариф и преврати ИИ-маркетолога\n"
        "в своего «штатного сотрудника», который всегда под рукой."
    )
    await send_split_text(message_obj, value_text, reply_markup=INLINE_TARIFFS_CTA)


async def send_full_version_pitch(message_obj, st: UserState):
    st.stage = "idle"
    st.chat_mode = False
    full_text = (
        "Полная версия ИИ-маркетолога 360 —\n"
        "это не «ещё одна нейросеть», а рабочий инструмент,\n"
        "который закрывает весь цикл маркетинга:\n\n"
        "• от идеи и стратегии → до готовых текстов и креативов;\n"
        "• от хаотичных постов → до системного контент-плана;\n"
        "• от «делаю на ощущениях» → до чёткой воронки.\n\n"
        "На платных тарифах ты получаешь:\n"
        "• стратегии и воронки под твой бизнес;\n"
        "• контент-планы и тексты;\n"
        "• изображения, видео, презентации (на продвинутых тарифах);\n"
        "• всё в одном сервисе, без команды из 3–5 человек.\n\n"
        "По деньгам:\n"
        "— дешевле найма маркетолога,\n"
        "— ИИ работает каждый день."
    )
    await send_split_text(message_obj, full_text, reply_markup=INLINE_TARIFFS_CTA)


async def handle_diag_choice_input(update: Update, context: ContextTypes.DEFAULT_TYPE, txt: str):
    user = update.effective_user
    st = get_state(user.id)
    chat_id = update.effective_chat.id if update.effective_chat else None
    normalized = txt.lower()
    if "демо" in normalized:
        await run_demo_after_diagnostic(update.message, st, bot=context.bot, chat_id=chat_id)
        return
    if "пол" in normalized or "верс" in normalized or "тариф" in normalized or "куп" in normalized:
        await send_full_version_pitch(update.message, st)
        return
    await present_post_diag_choices(update.message, st)
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "diag_demo":
        await run_demo_after_diagnostic(q.message, st, bot=context.bot, chat_id=chat_id)
        return

    if data == "diag_full":
        await send_full_version_pitch(q.message, st)
        return
